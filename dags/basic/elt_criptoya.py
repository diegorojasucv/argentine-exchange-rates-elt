...
def elt_criptoya() -> None:
    ...
    transform_task = PythonOperator(
        ...
        op_kwargs={'data': "{{ ti.xcom_pull(task_ids='fetch_data_from_api') }}"},
    )

    load_task = PythonOperator(
        task_id='load_data_to_postgres',
        python_callable=load_data_to_postgres,
        op_kwargs={'df_json': "{{ ti.xcom_pull(task_ids='transform_data') }}", 'db_config': db_config},
    )
    ...
    fetch_task >> transform_task >> load_task >> dbt_task
